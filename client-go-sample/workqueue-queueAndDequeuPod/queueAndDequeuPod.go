...
func main() {
	...
	// Namespaceを限定したInformerFactoryを作成
	...
	podInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(old interface{}) {
			// この場合oldは *v1.Pod型 で渡される
			// Debug
			// log.Println("obj = ", old, " Type = ", reflect.TypeOf(old))

			var key string
			var err error
			// in-memory-cacheからnamespace/nameを取得(本当?)
			// APIオブジェクトをkey(namespace/name)に変換
	...
	// WorkQueueから値を取得
	for {
		log.Println("Get item from WorkQueue")
		obj, shutdown := queue.Get() // 取得が完了するまでここで待機

		log.Println("obj = ", obj, " ,shutdown = ", shutdown)

		queue.Forget(obj)
		queue.Done(obj)

		if shutdown {
			log.Println("WorkQueue is shutdown")
		}
